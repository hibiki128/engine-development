...
void DsvManager::Initialize(DXDevice *device) {
    ...
    // デスクリプタ1つ分のサイズを取得
    descriptorSize_ = device_->Get()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);

    // DSV用のヒープ。DSVはShader内で触るものではないので、ShaderVisibleはfalse
    heap_ = device_->CreateDescriptorHeap(D3D12_DESCRIPTOR_HEAP_TYPE_DSV, kMaxDSVCount, false);
}
...
D3D12_CPU_DESCRIPTOR_HANDLE DsvManager::Create(uint32_t index, ID3D12Resource *resource, DXGI_FORMAT format) {
    ...
    device_->Get()->CreateDepthStencilView(resource, &dsvDesc, handle);
    return handle;
}
