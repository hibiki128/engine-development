...
void DXCommandList::Initialize(DXDevice *pDevice, D3D12_COMMAND_LIST_TYPE type)
{
    HRESULT hr;
    ...
    {
        hr = pDevice->Get()->CreateCommandAllocator(type, IID_PPV_ARGS(&commandAllocators_[i]));
        assert(SUCCEEDED(hr));
    }
