...
void ShadowMap::Initialize()
{
    ...
    hr = pDxCommon_->GetDevice()->CreateDescriptorHeap(&dsvHeapDesc, IID_PPV_ARGS(&dsvHeap_));
    assert(SUCCEEDED(hr));
    ...
    pDxCommon_->GetDevice()->CreateDepthStencilView(depthResource_.Get(), &dsvDesc, dsvHandle_);

    // ----- SRV 作成 (R32_FLOAT) -----
    CreateShadowSRV();

    // ----- ShadowData 定数バッファ -----
    shadowDataResource_ = pDxCommon_->CreateBufferResource(sizeof(ShadowDataGPU));
    shadowDataResource_->Map(0, nullptr, reinterpret_cast<void **>(&pShadowDataPtr_));
...
{
    if (shadowDataResource_ && pShadowDataPtr_)
    {
        shadowDataResource_->Unmap(0, nullptr);
        pShadowDataPtr_ = nullptr;
    }
...
void ShadowMap::CreateShadowSRV()
{
    ...
    pDxCommon_->GetDevice()->CreateShaderResourceView(
        depthResource_.Get(), &srvDesc, pSrvManager_->GetCPUDescriptorHandle(srvIndex_));
}
...
void ShadowMap::EndShadowPass()
{
    ...
    barrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
    pDxCommon_->GetCommandList()->ResourceBarrier(1, &barrier);
...
Matrix4x4 ShadowMap::MakeLightViewMatrix() const
{
    ...
    Matrix4x4 view{};
    view.m[0][0] = right.x;
    view.m[0][1] = actualUp.x;
    view.m[0][2] = forward.x;
    view.m[0][3] = 0.f;
    view.m[1][0] = right.y;
    view.m[1][1] = actualUp.y;
    view.m[1][2] = forward.y;
    view.m[1][3] = 0.f;
    view.m[2][0] = right.z;
    view.m[2][1] = actualUp.z;
